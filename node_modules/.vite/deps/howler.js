		...
		HowlerGlobal.prototype = {
			...
			*/
			_setup: function() {
				...
				self._autoSuspend();
				if (!self.usingWebAudio) if (typeof Audio !== "undefined") try {
					var test = new Audio();
		...
		Howl.prototype = {
			...
			*/
			mute: function(muted, id) {
				...
				if (typeof id === "undefined") if (typeof muted === "boolean") self._muted = muted;
			...
			*/
			volume: function() {
				...
				else if (args.length === 1 || args.length === 2 && typeof args[1] === "undefined") if (self._getSoundIds().indexOf(args[0]) >= 0) id = parseInt(args[0], 10);
			...
			*/
			loop: function() {
				...
				else if (args.length === 1) if (typeof args[0] === "boolean") {
					loop = args[0];
					self._loop = loop;
				} else {
			...
			*/
			rate: function() {
				...
				else if (args.length === 1) if (self._getSoundIds().indexOf(args[0]) >= 0) id = parseInt(args[0], 10);
			...
			*/
			seek: function() {
				...
				var sound = self._soundById(id);
				if (sound) if (typeof seek === "number" && seek >= 0) {
	...
	(function() {
		...
		*/
		Howl.prototype.stereo = function(pan, id) {
			...
			if (typeof id === "undefined") if (typeof pan === "number") {
				self._stereo = pan;
				self._pos = [
					pan,
					0,
			...
			for (var i = 0; i < ids.length; i++) {
				...
				if (sound) if (typeof pan === "number") {
					...
					if (sound._node) {
						...
						if (pannerType === "spatial") if (typeof sound._panner.positionX !== "undefined") {
		...
		*/
		Howl.prototype.pos = function(x, y, z, id) {
			...
			if (typeof id === "undefined") if (typeof x === "number") self._pos = [
				x,
				y,
				z
			];
		...
		*/
		Howl.prototype.orientation = function(x, y, z, id) {
			...
			if (typeof id === "undefined") if (typeof x === "number") self._orientation = [
				x,
				y,
				z
			];
		...
		*/
		Howl.prototype.pannerAttr = function() {
			...
			else if (args.length === 1) if (typeof args[0] === "object") {
				o = args[0];
				if (typeof id === "undefined") {
